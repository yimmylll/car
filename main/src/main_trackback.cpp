...
/**********************************************/
    ...
    */
    ...
    while(ros::ok())
    {
        ...
        // 获取路径
        loop_rate.sleep();
        if(targetIndex>=pathSize && cloudPathValid == 1) { //自动驾驶模式，收点
            block = 0;//下发坐标默认初始无障碍
            compelStop1();//收点后等待 开始标志下发 才开始
            pstop = 1;//初始不刹车
            vector<double>().swap(path);
            path = getPath(0);//to do.................................
        ...
        if(back)
        {
            if(G_lostGps == false)
            {
                moveTargetBack(ilat, ilon, QR_lat, QR_lon, igpsangle, minDistance, BackgasV, brakeV);
                continue;
            }
            else
                printf('WARNING  BACK=TURE && G_lostGps == true')
        }
        /**************** end ************************************************/
		printf("**********************************\n");//每次小车启动且进入非倒车循环
