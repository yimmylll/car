...
int main(int argc, char **argv)
{
    ...
    int stop;
    int pstop=1;//初始不刹车
	int compelStop = 0;//强制停止标志，仅在手动模式下到达单点时生效，在什么模式下都应该控制小车停止
    int tu;
    int go=0;
    double ilat ;
    double ilon ;
...
//	int stop_DelayTimes = 301;//初始不刹车释放
    ...
    */
    ...
    while(ros::ok())
    {
		...
		if(manualMode == 1 && cloudPathValid == 1) {//手动前进模式
            ...
            path=getPath();
            pathSize=path.size()/2;
			printf("手动倒车pathsize:%d  path[0]:%8.4f  path[1]:%8.4f\n", pathSize, path[0], path[1]);//y,x 经、纬度
            if(pathSize > 0) {
                //ismanualBack = true;
				manualBack_go = 0;
        ...
        */
		...
		//检测云平台的停止信号
		if (compelStop == 0) {/////////走完单点后不要getstop，防止把走完单点后置stop=1给刷新成0
			stop = getStop();
		}
		else
		{
		...
		//检测云平台是否绕障碍
		tu=getTurn();
		//printf("manualMode:%d   start:%d   stop:%d   tu:%d\n", manualMode, start, stop, tu);
		...
		if (stop == 1 ) {
			...
			//if (stop_DelayTimes <= 300) {
			//	stop_DelayTimes++;
			//}
			//if (stop_DelayTimes == 300)//3s释放刹车
			//{
			//	//stop_DelayTimes = 0;
			//	printf("stop_DelayTimes == 300,brake(0.0) delay1s\n");
			//	brake(0.0);
			//	delay_1s(1);
			//}
			continue;
  ...
  //          continue;
        ...
        if(G_lostGps == true) {
            dis = getDistanceInMeter(G_car);
             printf("G_car.X:%f  Gcar.y:%f  G_car.dX:%f  Gcar.dy:%f\n",G_car.x, G_car.y,G_dx, G_dy);
        } else {
